...
def test_release_tag_parser_requires_exact_semantic_v_prefix() -> None:
    assert parse_release_tag("v0.1.1").version == "0.1.1"
    assert validate_release_tag("v0.1.1").version == __version__
    assert release_tag_matches_version("v0.1.1")
    assert not release_tag_matches_version("v0.1.0", expected_version=__version__)

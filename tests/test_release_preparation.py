"""Published release documentation and workflow contracts for v0.1.2."""
...
def test_published_version_and_notes_are_bilingual_and_conservative() -> None:
    ...
    assert "## 中文" in notes
    for phrase in (
        "published desktop usability hotfix release",
        "Braid setup / preview",
        "Calculation / results",
        "Manual braid input",
...
def test_readme_and_distribution_describe_published_state_and_stable_assets() -> None:
    ...
    assert "`v0.1.2` tag and GitHub Release are published" in distribution

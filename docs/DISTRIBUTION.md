...
The maintained application version is `0.1.4`, sourced only from
`src/version.py`. The tag-driven Windows workflow validates that an exact
`v0.1.4` tag matches that version before it builds, smoke-tests, and publishes
the installer, portable ZIP, checksum manifest, and maintained release notes.
...
- [ ] On a clean Windows machine, download the `v0.1.4` installer and `SHA256SUMS.txt`; independently verify the installer hash.
...
signing is not currently enabled; the maintained release line is `v0.1.4`.

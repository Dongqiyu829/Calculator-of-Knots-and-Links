"""Compute one EYB-normalized output for the sl2 fundamental branch."""
...
from src.invariants.eyb_invariant import build_sl2_fundamental_eyb_data, compute_eyb_invariant
from src.rmatrix.sl2_rmatrix import build_sl2_fundamental_rmatrix


def main() -> None:
    """Compute and print one sl2 fundamental EYB-normalized example."""

    q = sp.Symbol("q", nonzero=True)
    braid_word = BraidWord.from_iterable(num_strands=2, generators=[1, 1, 1], label="2-strand cubic braid")
    rdata = build_sl2_fundamental_rmatrix(q)
    eyb_data = build_sl2_fundamental_eyb_data(q)
    ...
    print("=== sl2 fundamental EYB demo ===")
